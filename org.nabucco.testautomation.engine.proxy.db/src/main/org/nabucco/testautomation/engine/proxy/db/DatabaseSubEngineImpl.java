...
import org.nabucco.testautomation.engine.proxy.db.DatabaseEngine;
import org.nabucco.testautomation.engine.proxy.db.DatabaseEngineOperationType;
...
public class DatabaseSubEngineImpl extends AbstractSubEngine implements DatabaseEngine {
	...
	public ActionResponse executeSubEngineOperation(
		...
		case DATABASE:
			return getDatabase().execute(context, propertyList, metadataList,
					actionType);
		case SQL_STATEMENT:
			return getSqlStatement().execute(context, propertyList,
					metadataList, actionType);
		default:
			String error = "Unsupported DatabaseEngineOperationType = '"
					+ operationType + "'";
			logger.error(error);
