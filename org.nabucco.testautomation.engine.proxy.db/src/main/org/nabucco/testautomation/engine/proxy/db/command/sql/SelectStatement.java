...
public class SelectStatement extends AbstractSQLCommand {
     ...
     */
    @Override
    public PropertyList execute(Metadata metadata, PropertyList propertyList) throws SQLCommandException {
    ...
    private PropertyList createResultProperty(ResultSet rs) throws SQLException, SQLCommandException {
        ...
        while (rs.next()) {
            PropertyList resultProperty = PropertyHelper.createPropertyList("" + ++result);
            PropertyHelper.add(resultProperty, resultProperties);

            for (int i = 1; i <= columns; i++) {
                Property rowProperty = createProperty(rs.getObject(i), columnTypes[i], columnNames[i]);
                if (rowProperty != null) {
    ...
    private Property createProperty(Object obj, int type, String name) throws SQLCommandException {
        ...
        case Types.INTEGER: {
            ...
            if (obj instanceof Long) {
                p = PropertyHelper.createNumericProperty(name, BigDecimal.valueOf((Long) obj));
            } else {
                p = PropertyHelper.createNumericProperty(name, (Integer) obj);
            }
            return p;
        }
        case Types.BOOLEAN: {
            BooleanProperty p = PropertyHelper.createBooleanProperty(name, (Boolean) obj);
            return p;
        }
        case Types.VARCHAR: {
            TextProperty p = PropertyHelper.createTextProperty(name, (String) obj);
            return p;
        }
        case Types.CHAR: {
            TextProperty p = PropertyHelper.createTextProperty(name, (String) obj);
        ...
        case Types.DATE: {
            DateProperty p = PropertyHelper.createDateProperty(name, (Date) obj);
            return p;
        }
        case Types.DOUBLE:
        case Types.DECIMAL:
        ...
        case Types.BIGINT: {
            ...
            if (obj == null) {
                p = PropertyHelper.createNumericProperty(name, (BigDecimal) null);
            } else if (obj instanceof Long) {
                long value = (Long) obj;
                p = PropertyHelper.createNumericProperty(name, BigDecimal.valueOf(value));
                ...
                p = PropertyHelper.createTextProperty(name, obj.toString());
                this.warning("Unmapped java-type found: "
                        + obj.getClass().getName() + ". Mapped to TextProperty ['" + name + "', '" + obj.toString()
                        + "']");
            }
            return p;
        }
        case Types.TIMESTAMP: {
            ...
            if (obj == null) {
                ...
                p = PropertyHelper.createDateProperty(name, new Date(((Timestamp) obj).getTime()));
            }
            return p;
        }
        ...
        default: {
            throw new SQLCommandException("No mapping for SQL Type: "
                    + type + " available. Skippping column name='" + name + "'");
        }
        }
    }
