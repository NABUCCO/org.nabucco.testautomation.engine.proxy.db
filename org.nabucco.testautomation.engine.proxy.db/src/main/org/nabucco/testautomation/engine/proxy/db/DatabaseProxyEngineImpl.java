...
public class DatabaseProxyEngineImpl extends AbstractProxyEngine {
    ...
    protected void configure(ProxyEngineConfiguration config) throws ProxyConfigurationException {
        try {
            this.databaseClassloader = new DatabaseClassloader(getProxySupport().getDeployPath());
            logger.info("DatabaseProxyEngine configured");
        } catch (ProxyConfigurationException ex) {
            throw ex;
        } catch (Exception ex) {
            throw new ProxyConfigurationException("Could not configure proxy", ex);
        }
    }
     ...
     */
    @Override
    protected DatabaseProxyConfiguration getProxyConfiguration(ProxyConfiguration configuration) {
        DatabaseProxyConfiguration config = new DatabaseProxyConfigImpl(getProxySupport().getProxyClassloader(),
                configuration);
        return config;
    }
